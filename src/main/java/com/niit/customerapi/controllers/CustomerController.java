...
*/
	...
	/*
	 * @PutMapping("/v1.0") public ResponseEntity<ResponseWrapper>
	 * updateCustomer(@RequestParam("accountNo") long
	 * accountNo,@RequestParam("newEmail") String newEmail){ Customer customer=
	 * customerService.updateCustomer(accountNo, newEmail);
	 * 
	 * if(customer != null){ return ResponseEntity.status(HttpStatus.OK).body(new
	 * ResponseWrapper<Customer>(customer)); }else return
	 * ResponseEntity.status(HttpStatus.NOT_FOUND).body(new
	 * ResponseWrapper("Customer not updated"));
	 * 
	 * 
	 * }
	 */
